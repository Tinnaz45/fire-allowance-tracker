...
| Concern | Current | Target | Gap |
|---|---|---|---|
| Rate source of truth | `fat.user_rates` per-user overrides (kilometre_rate, small/large/double/spoilt/delayed/overnight/standby_night meal, …); defaults in [lib/calculations/defaultRates.js](lib/calculations/defaultRates.js) | `fat.rates` (global codes) + `fat.rate_versions` (append-only, `effective_from`); UI surfaces global rates only | **Replace entirely.** Per-user rate overrides are not in the canonical model. Settings UI becomes admin-only writes to `rate_versions`; users no longer override rates. |
...
| Open unresolved rates | `delayedMealAllowance` UNRESOLVED, `standbyNightMealAllowance` UNCONFIRMED, `retainAllowancePerHour` unresolved | Same gaps; canonical does not resolve them | **Carry forward.** These remain `TODO` in `ENTITLEMENT_RULES_v1.0.md`. |
