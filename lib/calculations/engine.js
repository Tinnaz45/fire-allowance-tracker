 ...
 *
 * Retain is hours-only: generated_hours is derived from shift + bookedOffTime
 * via calcRetainHours(). There is no configurable hourly rate and no derived
 * retain dollar amount. Meal allowance is auto-derived via
 * calcRetainMealEligibility(). overnightCash is the only user-entered dollar
 * field (a per-claim cash imprest, not a rate).
 *
 * @param {object} params
 * @param {'Day'|'Night'} params.shift
 * @param {string}        params.bookedOffTime
...
export function calcRetainClaim({ shift, bookedOffTime, overnightCash }, rates = {}) {
  ...
  const total = roundMoney(overnight + mealAmount)
  return {
    generatedHours,
 ...
 *
 * Hours-only: explains the rule path used (pre-trigger, mandatory minimum,
 * post-flat-end, capped). Retain has no dollar amount; existing
 * meal-eligibility + overnight-cash framing intact.
 */
