...
export function buildRecallCalcLines({ distHomeKm, distStnKm, mealEntitlement }, rates, labels = {}) {
  ...
  if (meal > 0) {
    ...
    const small = safeNum(rates.smallMealAllowance)
    const large = safeNum(rates.largeMealAllowance)
    const mealRate = mealEntitlement === 'small'  ? small
                   : mealEntitlement === 'large'  ? large
                   : small + large
    lines.push('── Meal ──')
    lines.push(`${mealLabel} = $${mealRate.toFixed(2)}`)
    if (mealEntitlement === 'double') {
      lines.push(`  (= $${small.toFixed(2)} small + $${large.toFixed(2)} large; counts as 1 small + 1 large for tax)`)
    }
  }
 ...
 * via calcRetainHours(); generated_amount = generated_hours × retainHourlyRate.
 * Meal allowance is auto-derived via calcRetainMealEligibility(). overnightCash
 * is the only user-entered dollar field (a per-claim cash imprest, not a rate).
 *
 * @param {object} params
 * @param {'Day'|'Night'} params.shift
...
export function calcRetainClaim({ shift, bookedOffTime, overnightCash }, rates = {}) {
  ...
  const hourlyRate      = safeNum(rates.retainHourlyRate)
 ...
 * post-flat-end, capped) and then derives the dollar amount from the hourly
 * rate snapshot. Existing meal-eligibility + overnight-cash framing intact.
 */
export function buildRetainCalcLines({ shift, bookedOffTime, overnightCash }, rates) {
  ...
  lines.push('── Retain $ (derived) ──')
  lines.push(`${result.generatedHours.toFixed(2)}h × $${result.retainHourlyRate.toFixed(2)}/h = $${result.retainAmount.toFixed(2)}`)
  if (result.retainHourlyRate <= 0) {
    lines.push('  (retain hourly rate not set — set it in Settings → Rates to derive a dollar amount)')
  }
...
export function createRateSnapshot(rates) {
  ...
  return {
    ...
    retainHourlyRate:     safeNum(rates?.retainHourlyRate),
