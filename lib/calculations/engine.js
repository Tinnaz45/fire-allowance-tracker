...
/**
 * Format a canonical FY label for display.
 * The stored/canonical value is compact (e.g. '2027FY'); this is the single
 * source of truth for turning it into a human-readable span.
 * e.g. '2027FY' → 'FY 2026 - 2027' (Australian FY: 1 July → 30 June).
 *
 * Defensive: any value that is not a canonical 'NNNNFY' label (null, '',
 * 'All Years', an already-formatted string, etc.) is returned unchanged, so
 * this is safe to apply at every display site.
 *
 * @param {string} label e.g. '2027FY'
 * @returns {string} e.g. 'FY 2026 - 2027'
 */
export function formatFYLabel(label) {
  if (typeof label !== 'string') return label
  const match = label.match(/^(\d{4})FY$/)
