...
const TEST_RATES = {
  ...
  retainHourlyRate:   20.00, // test-only rate (real rate set per-user in Settings)
}
...
const SCENARIOS = [
  ...
  // ── Retain (hours-first FRV calc) ────────────────────────────────────────────
  // retainHourlyRate in TEST_RATES is $20.00/h, used to derive dollar amounts.
  ...
  },
  {
    ...
    run: () => check('retain min trigger', calcRetainClaim({
      ...
      generatedHours: 4.00,
      retainAmount: 80.00,             // 4.00 × 20
      overnightCash: 45.00,
      totalAmount: 125.00,             // 80 + 45 (mid-flat window → no meal)
    }),
  },
  {
