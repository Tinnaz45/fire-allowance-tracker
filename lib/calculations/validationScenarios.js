...
const SCENARIOS = [
  ...
  // ── Retain (hours-first FRV calc, Maint Stn N/N $) ───────────────────────────
  // Retain $ = generated_hours × canonical overtime rate ($101.0225/h). The
  // rate is fixed (not user-editable) so dollar expectations are exact.

  {
    ...
    run: () => check('retain pre-trigger', calcRetainClaim({
      ...
      retainHourlyRate: 101.0225,
      retainAmount: 50.51,             // 0.50 × 101.0225 → 50.51
      overnightCash: 0,
      totalAmount: 50.51,
    }),
  },
  {
    name: 'calcRetainClaim: mandatory min at exact trigger (4.00h = $404.09)',
    run: () => check('retain min trigger', calcRetainClaim({
      shift: 'Day', bookedOffTime: '19:00', overnightCash: 45.00,
    }, TEST_RATES), {
      generatedHours: 4.00,
      retainAmount: 404.09,            // 4.00 × 101.0225 = 404.09 (Maint Stn N/N)
      overnightCash: 45.00,
      totalAmount: 469.64,             // 404.09 + 45 overnight + 20.55 large meal (Day 19:00 → 1× Large)
    }),
  },
  {
    name: 'calcRetainClaim: post-flat-end Day 22:15 (4.25h)',
    run: () => check('retain post-flat-end 4.25h', calcRetainClaim({
      shift: 'Day', bookedOffTime: '22:15', overnightCash: 0,
    }, TEST_RATES), {
      generatedHours: 4.25,            // 4.00 min + 0.25 (15 min past 22:00)
      retainAmount: 429.35,            // 4.25 × 101.0225 → 429.35
    }),
  },
  {
    ...
    run: () => check('retain cross-midnight', calcRetainClaim({
      ...
      generatedHours: 6.50,
      retainAmount: 656.65,            // 6.50 × 101.0225 → 656.65
    }),
  },
  {
    name: 'calcRetainClaim: 1.25h direct rate check (= $126.28)',
    // Synthetic: verifies the canonical rate × 1.25h independent of hour rules.
    run: () => checkValue('retain 1.25h × $101.0225',
      roundMoney(1.25 * 101.0225), 126.28),
  },
  {
