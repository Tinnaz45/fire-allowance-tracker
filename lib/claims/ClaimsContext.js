...
export function ClaimsProvider({ children }) {
  ...
  const groupedView = (() => {
    ...
    // Retain-parent dedupe. The parent retain row is an operational record
    // whose dollar fields (retain_amount, overnight_cash, total_amount) are
    // also carried by the auto-children (Maint stn N/N, Overnight cash,
    // Retain meal). Summing both rows would double-count, so when auto-
    // children exist, zero the parent's dollar contribution and let the
    // children carry the payable totals. This mirrors the standby parent
    // (which is written with total_amount = 0 from the start) and handles
    // historical retain rows that were written before this dedupe existed.
    // Operational metadata (date, shift, hours, rate, snapshot) is preserved
    // so the parent still functions as the canonical operational record.
    for (const groupId of Object.keys(childrenByGroup)) {
      const arr = childrenByGroup[groupId]
      const hasRetainAutoChild = arr.some(
        (c) => c.claimType === 'retain' && c.calculation_inputs?.autoChild != null
      )
      if (!hasRetainAutoChild) continue
      childrenByGroup[groupId] = arr.map((c) => {
        const isRetainParent = c.claimType === 'retain' && c.calculation_inputs?.autoChild == null
