...
function getAutoChildDefinitions(claimType, parentId, userId, date, financialYearId, fields, breakdown, ratesSnapshot) {
  ...
  if (claimType === 'retain') {
    // Maint stn N/N payout — hours-first FRV retain. Carries both
    // generated_hours and the rate snapshot used so the dollar amount is
    // reproducible from the row alone.
    if ((breakdown.generatedHours ?? 0) > 0 || breakdown.retainAmount > 0) {
      children.push({
        table: 'retain',
        row: {
          user_id:          userId,
...
function buildRetainRow(userId, date, breakdown, fields, ratesSnapshot, financialYearId, claimNumber, claimGroupId) {
  return {
    ...
    shift:            fields.shift          || null,
    booked_off_time:  fields.bookedOffTime  || null,
    // Hours-first FRV calc. retain_amount is derived = hours × rate snapshot.
    generated_hours:  breakdown.generatedHours ?? 0,
    retain_rate_used: breakdown.retainHourlyRate ?? 0,
    retain_amount:    breakdown.retainAmount,
    overnight_cash:   0,
