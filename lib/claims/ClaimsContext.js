...
import { mirrorClaimToCanonical } from '@/lib/claims/canonicalBridge'
import { isOperationalContainerRow } from '@/lib/claims/claimMeta'
...
function getAutoChildDefinitions(claimType, parentId, userId, date, financialYearId, fields, breakdown, ratesSnapshot) {
  ...
  if (claimType === 'recalls') {
    // Recall meal entitlement is auto-derived in calcRecallClaim() —
    // breakdown.mealEntitlement / smallCount / largeCount are the single
    // source of truth here. The old manual `fields.mealEntitlement` and
    // operational reconciliation metadata (incident_number, payslip_pay_nbr)
    // are no longer collected at claim-creation time.
    const mealEntitlement = breakdown.mealEntitlement || 'none'
    ...
    // Petty cash meal — driven by the auto-derived entitlement.
    if (mealEntitlement !== 'none' && breakdown.mealieAmount > 0) {
      children.push({
        table: 'spoilt_meals',
        row: {
          user_id:          userId,
          date,
          status:           'Pending',
          meal_type:        mealEntitlement === 'double' ? 'Double' : 'Large',
  ...
  if (claimType === 'retain') {
    // Maint stn N/N payout — hours-first FRV retain. Carries generated_hours
    // and the canonical overtime rate snapshot so the dollar amount
    // (generated_hours × retain_rate_used) is reproducible from the row alone.
    if ((breakdown.generatedHours ?? 0) > 0) {
      children.push({
        table: 'retain',
        row: {
          user_id:          userId,
          ...
          generated_hours:  breakdown.generatedHours    ?? 0,
          retain_rate_used: breakdown.retainHourlyRate  ?? 0,
          retain_amount:    breakdown.retainAmount      ?? 0,
          overnight_cash:   0,
          total_amount:     breakdown.retainAmount      ?? 0,
          financial_year_id: financialYearId,
          claim_group_id:   parentId,
          rates_snapshot:   ratesSnapshot,
          calculation_inputs: {
            autoChild:        'maint_stn_nn',
            generatedHours:   breakdown.generatedHours    ?? 0,
            retainHourlyRate: breakdown.retainHourlyRate  ?? 0,
            retainAmount:     breakdown.retainAmount      ?? 0,
            retainRulePath:   breakdown.retainRulePath    || null,
          },
  ...
  if (claimType === 'standby' || claimType === 'md') {
    ...
    // Excess Travel — km × rate.
    //   • Standby: dist_km = rostered↔standby round-trip km.
    //   • M&D: dist_km = payable km = max(0, Home→Rostered − Home→M&D) (no ×2);
    //     both source distances are preserved in calculation_inputs so the
    //     breakdown/DB/petty-cash export all reproduce the same figure.
    // Pay Number is collected at mark-as-paid time, not at create time.
    if (breakdown.travelAmount > 0) {
      const isMD = (fields.standbyType || 'Standby') === 'M&D'
      const excessCalcInputs = { autoChild: 'standby_excess_travel', distKm: Number(fields.distKm) || 0 }
      if (isMD) {
        excessCalcInputs.mdRule           = 'home_rostered_minus_home_md'
        excessCalcInputs.homeToRosteredKm = fields.mdHomeToRosteredKm ?? null
        excessCalcInputs.homeToMdKm       = fields.mdHomeToMdKm ?? null
        excessCalcInputs.payableKm        = fields.mdPayableKm ?? (Number(fields.distKm) || 0)
      }
      children.push({
        table: 'standby',
        row: {
          ...
          claim_group_id:   parentId,
          rates_snapshot:   ratesSnapshot,
          calculation_inputs: excessCalcInputs,
          _child_label:     'Excess Travel',
        },
      })
    }
...
function buildRecallRow(userId, date, breakdown, fields, ratesSnapshot, financialYearId, claimNumber, claimGroupId, routingMeta) {
  ...
  return {
    ...
    recall_stn_label:   fields.recallStn        || null,
    rostered_stn_label: fields.rosteredStn      || null,
    arrived:            fields.arrivalTime      || null,
    ...
    calculation_inputs: {
      ...
      // Auto-derived entitlement (single source of truth — replaces the old
      // user-selected `fields.mealEntitlement`).
      shift:            fields.shift          || null,
      notified:         !!fields.notified,
      arrivalTime:      fields.arrivalTime    || null,
      bookedOffTime:    fields.bookedOffTime  || null,
      mealEntitlement:  breakdown.mealEntitlement || 'none',
      smallMealCount:   breakdown.smallCount  ?? 0,
      largeMealCount:   breakdown.largeCount  ?? 0,
      mealTier:         breakdown.mealTier    || 'none',
      mealReason:       breakdown.mealReason  || null,
      durationMin:      breakdown.durationMin ?? null,
...
function buildRetainRow(userId, date, breakdown, fields, ratesSnapshot, financialYearId, claimNumber, claimGroupId) {
  return {
    ...
    shift:            fields.shift          || null,
    booked_off_time:  fields.bookedOffTime  || null,
    // Hours-first FRV calc. retain_amount (Maint Stn N/N) is derived =
    // generated_hours × canonical overtime rate snapshot.
    generated_hours:  breakdown.generatedHours ?? 0,
    retain_rate_used: breakdown.retainHourlyRate ?? 0,
    retain_amount:    breakdown.retainAmount ?? 0,
    overnight_cash:   0,
    ...
    claim_group_id:   claimGroupId     || null,
    calculation_inputs: {
      // Hours-first inputs (canonical source of truth for reproducing the calc)
      shift:            fields.shift         || null,
      bookedOffTime:    fields.bookedOffTime || null,
      generatedHours:   breakdown.generatedHours    ?? 0,
      retainRulePath:   breakdown.retainRulePath    || null,
      retainExplanation: breakdown.retainExplanation || null,
      retainHourlyRate: breakdown.retainHourlyRate  ?? 0,
      retainAmount:     breakdown.retainAmount      ?? 0,
...
function buildStandbyRow(userId, date, breakdown, fields, ratesSnapshot, financialYearId, claimNumber, claimGroupId, routingMeta) {
  ...
  return {
    ...
    standby_type:     fields.standbyType || 'Standby',
    shift:            fields.shift || 'Day',
    // Operational-context capture (Recall-style). rostered_stn_label is the
    // editable rostered station; standby_stn_label is the destination/event
    // station (Standby or M&D). Both are metadata only — they never affect
    // amounts, which remain entirely on the auto-generated child rows.
    rostered_stn_label: fields.rosteredStn || null,
    standby_stn_label:  fields.standbyStn  || null,
    dist_km:          0,
    travel_amount:    0,
    night_mealie:     0,
    total_amount:     0,
    adjusted_amount:  null,
    // NOTE: payment-method tagging lives on the canonical fat.claim_entitlements
    // schema (generated by the entitlement engine), NOT on the prototype
    // fat.standby row — that column does not exist here. total_amount = 0 keeps
    // this operational/entitlement marker out of payslip dollar totals.
    calc_snapshot:    breakdown.calcSnapshot ?? null,
    ...
    calculation_inputs: {
      ...
      arrivedTime,
      rosteredStn: fields.rosteredStn || null,
      standbyStn:  fields.standbyStn || null,
      routing:     standby || null,
      // M&D petty-cash km rule provenance: payable km = max(0, A − B),
      // A = Home→Rostered, B = Home→M&D. Preserved on the parent so the claim
      // is reproducible from the operational row alone. Null for Standby.
      ...(fields.standbyType === 'M&D'
        ? {
            mdRule:           'home_rostered_minus_home_md',
            homeToRosteredKm: fields.mdHomeToRosteredKm ?? null,
            homeToMdKm:       fields.mdHomeToMdKm ?? null,
            payableKm:        fields.mdPayableKm ?? (Number(fields.distKm) || 0),
          }
        : {}),
...
function buildSpoiltRow(userId, date, breakdown, fields, ratesSnapshot, financialYearId, claimNumber, claimGroupId) {
  return {
    ...
    shift:            fields.shift || 'Day',
    // Operational-context capture for meal claims. operational_stn_label is the
    // station where the meal event occurred; platoon is the user-selectable FRV
    // platoon (defaults to the auto-resolved value — see addClaim). Both are
    // metadata only and never affect the meal amount.
    operational_stn_label: fields.operationalStn || null,
    platoon:          fields.platoon || null,
    // Required 5-digit firecall number, validated by ClaimForm before submit.
    // Metadata only — never affects the meal amount.
    call_number:      fields.firecallNumber || null,
    meal_amount:      breakdown.mealAmount,
    total_amount:     breakdown.totalAmount,
    adjusted_amount:  breakdown.adjustedAmount ?? null,
    calc_snapshot:    breakdown.calcSnapshot ?? null,
    incident_time:    fields.incidentTime    || null,
    meal_interrupted: null,
    return_to_stn:    null,
    rates_snapshot:   ratesSnapshot,
    financial_year_id: financialYearId || null,
    claim_number:     claimNumber      || null,
    ...
    calculation_inputs: {
      ...
      shift:          fields.shift    || 'Day',
      operationalStn: fields.operationalStn || null,
      firecallNumber: fields.firecallNumber || null,
    },
  }
}
...
export function ClaimsProvider({ children }) {
  ...
  const addClaim = useCallback(async ({ userId, claimType, date, breakdown, fields, rates, financialYearId, routingMeta = null }) => {
    ...
    try {
      ...
      // Recall claims no longer carry incident_number at creation time —
      // reconciliation metadata is captured downstream, never on the
      // operational claim form. Other claim types still pass it through.
      ...
      if (financialYearId && claimNumber) {
        const group = await createClaimGroup(
          ...
          claimNumber,
          date,
          claimType === 'recalls' ? null : (fields.incidentNumber || null)
        )
      ...
      // Auto-resolve the operational FRV platoon from the claim's date + shift
      // (deterministic 2026 rotation — single source of truth). Stamped onto the
      // row's `platoon` column AND its calculation_inputs snapshot so the
      // resolved platoon is preserved historically, immune to any future change
      // in the rotation engine. Universal across all claim types; null only when
      // a shift hasn't been chosen (e.g. a Recall mid-edit).
      // A row builder may already have set `platoon` from an explicit user
      // selection (currently the meal claims, which expose a platoon picker).
      // Respect that override; otherwise fall back to the deterministic
      // auto-resolved platoon. When neither is present (e.g. a shift not yet
      // chosen) the column stays null, exactly as before.
      const operationalPlatoon = resolveOperationalPlatoon(date, resolvedFields.shift)
      const effectivePlatoon = row.platoon || operationalPlatoon
      if (effectivePlatoon) {
        row.platoon = effectivePlatoon
        if (row.calculation_inputs && typeof row.calculation_inputs === 'object') {
          row.calculation_inputs.platoon = effectivePlatoon
        }
      }
      ...
      if (claimGroupId) {
        ...
        for (const { table, row: childRow } of childDefs) {
          ...
          // Children inherit the parent event's effective platoon (explicit
          // override or auto-resolved) so the resolved value is consistent
          // across the whole claim group.
          if (effectivePlatoon) insertRow.platoon = effectivePlatoon
      ...
      // Step 4.5: Mirror Standby / M&D claims into the canonical entitlement
      // schema. Purely additive and STRICTLY non-fatal — the prototype claim is
      // already fully persisted above; canonical generation runs in parallel and
      // must never roll back or block the live save. mirrorClaimToCanonical
      // swallows its own errors; the extra guard here is defence-in-depth
      // against an unexpected throw (e.g. a module-load failure).
      if (claimType === 'standby' || claimType === 'md') {
        try {
          await mirrorClaimToCanonical({
            userId,
            claimType,
            date,
          ...
          console.warn('[Claims] Canonical mirror warning (non-fatal):', canonicalErr)
        }
      }
  ...
  // Updates date, total_amount, status, and (optionally) shift + platoon.
  // Does NOT recalculate amounts from rates — preserves the original calculation.
  // shift/platoon are operational metadata only (they never affect dollar amounts
  // or payment state). When the edited claim belongs to a group, the new shift +
  // platoon are propagated to every row in that group so the whole claim stays
  // internally consistent. After updating a child, recomputes the parent status.

  const updateClaim = useCallback(async ({ userId, claim, date, amount, status, shift, platoon, financialYearId }) => {
    if (mutating.current) throw new Error('A save is already in progress. Please wait.')
    mutating.current = true
    ...
    try {
      ...
      const payload = { date, [amountColumn]: Number(amount), status }
      // Only touch shift/platoon when the caller supplied a shift value (the
      // edit form always does; other callers may omit it to leave them as-is).
      const includeMeta = shift !== undefined
      if (includeMeta) {
        payload.shift   = shift || null
        payload.platoon = platoon || null
      }
      ...
      // Propagate shift/platoon across the whole claim group so every sub-claim
      // row carries the same operational metadata. A group's rows all live in a
      // single table, but updating each candidate table keyed by claim_group_id
      // is harmless (no-op where there is no match) and avoids stale state.
      if (includeMeta && claim.claim_group_id) {
        for (const table of CLAIM_TABLES) {
          const { error: metaError } = await fat
            .from(table)
            .update({ shift: shift || null, platoon: platoon || null })
            .eq('claim_group_id', claim.claim_group_id)
          if (metaError) {
            console.warn(`[Claims] shift/platoon propagation warning (${table}):`, metaError)
          }
        }
      }
  ...
  // ── Delete an entire claim (parent group + all sub-claims) ────────────────
  // Permanent. claim_group_id is ON DELETE SET NULL, so child rows must be
  // deleted explicitly first — deleting only the parent would orphan them.
  // Cleans the payment_components ledger for every removed row before deleting.

  const deleteClaimGroup = useCallback(async ({ userId, group, financialYearId }) => {
    if (mutating.current) throw new Error('A save is already in progress. Please wait.')
    mutating.current = true

    try {
      // Delete child rows table-by-table, clearing the ledger first.
      for (const table of CLAIM_TABLES) {
        const { data: rows, error: selErr } = await fat
          .from(table)
          .select('id')
        ...
        await deletePaymentComponents(table, ids)

        const { error: delErr } = await fat
          .from(table)
          .delete()
          .eq('claim_group_id', group.id)
          .eq('user_id', userId)
      ...
      // Delete the parent group row.
      const { error: groupErr } = await fat
        .from('claim_groups')
        .delete()
        .eq('id', group.id)
        .eq('user_id', userId)
  ...
  // ── Delete a single sub-claim (entitlement row) ───────────────────────────
  // Permanent. Removes one child row, clears its ledger entry, then keeps the
  // parent group intact — recomputing its status, or deleting the now-empty
  // parent group if this was its last child.

  const deleteSubClaim = useCallback(async ({ userId, claim, financialYearId }) => {
    if (mutating.current) throw new Error('A save is already in progress. Please wait.')
    mutating.current = true
    ...
    try {
      ...
      await deletePaymentComponents(table, [claim.id])
      ...
      if (claim.claim_group_id) {
        const remaining = await countGroupChildren(claim.claim_group_id)
        if (remaining === 0) {
          await fat
            .from('claim_groups')
            .delete()
            .eq('id', claim.claim_group_id)
            .eq('user_id', userId)
        } else {
          await recomputeGroupStatus(claim.claim_group_id, userId, financialYearId)
        }
  ...
  // ── Payment ledger cleanup (best-effort) ──────────────────────────────────
  // Removes fat.payment_components rows for the given claim table + ids. The
  // ledger table is created by the v3 payment migration; if absent, this is
  // silently skipped so deletion still succeeds.

  async function deletePaymentComponents(claimTable, claimIds) {
    if (!claimIds || claimIds.length === 0) return
    try {
      const { error } = await fat
        .from('payment_components')
        .delete()
        .eq('claim_table', claimTable)
        .in('claim_id', claimIds)
      if (error) console.warn('[Claims] Payment component cleanup warning:', error)
    } catch (err) {
      console.warn('[Claims] Payment component cleanup warning:', err)
    }
  }

  // ── Count remaining children of a group across all claim tables ────────────

  async function countGroupChildren(groupId) {
    const results = await Promise.all(
      CLAIM_TABLES.map((table) =>
        fat
          .from(table)
  ...
  const groupedView = (() => {
    ...
    // Operational-container parent dedupe. A Recall or Retain parent row is
    // written with the full claim total, and its auto-children each carry a
    // split of those same dollars (Callback-Ops / Excess Travel / Petty cash
    // meal for recalls; Maint stn N/N / Overnight cash / Retain meal for
    // retain). Summing the parent and its children double-counts, so when auto-
    // children exist, zero the container parent's dollar contribution and let
    // the children carry the payable totals. This mirrors the Standby/M&D parent
    // (written with total_amount = 0 from the start, and never a container — it
    // carries its own autoChild slug) and repairs historical Recall/Retain rows
    // written before this dedupe existed, so no data backfill is required.
    // Operational metadata (date, shift, stations, hours, rate, snapshot) is
    // preserved so the parent still functions as the canonical operational
    // record. This single pass is the authoritative dollar dedupe — every dollar
    // aggregator (Financial Snapshot, group cards, reconciliation, exports) reads
    // from the deduped children; the tax page applies the equivalent parent-only
    // rule for km/meal counts in calcTaxSummary (see lib/claims/claimMeta.js).
    for (const groupId of Object.keys(childrenByGroup)) {
      const arr = childrenByGroup[groupId]
      childrenByGroup[groupId] = arr.map((c) => {
        if (!isOperationalContainerRow(c, arr)) return c
        return {
          ...c,
          travel_amount:    0,
          mealie_amount:    0,
