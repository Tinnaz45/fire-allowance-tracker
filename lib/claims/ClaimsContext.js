...
export function ClaimsProvider({ children }) {
  ...
  const addClaim = useCallback(async ({ userId, claimType, date, breakdown, fields, rates, financialYearId, routingMeta = null }) => {
    ...
    try {
      ...
      if (operationalPlatoon) {
        row.platoon = operationalPlatoon
        if (row.calculation_inputs && typeof row.calculation_inputs === 'object') {
          row.calculation_inputs.platoon = operationalPlatoon
        }
      }
      ...
      if (claimGroupId) {
        ...
        for (const { table, row: childRow } of childDefs) {
          ...
          // Children inherit the parent event's operational platoon so the
          // resolved value is consistent across the whole claim group.
          if (operationalPlatoon) insertRow.platoon = operationalPlatoon
