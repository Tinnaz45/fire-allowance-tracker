...
//
// Station mapping: the prototype carries station *labels* ("FS43 - Deer Park").
// parseStationInput extracts the numeric id from the canonical label shape.
// The id is validated against fat.stations before use so a free-text fragment
// can never produce a dangling FK (station_id_snapshot / *_station_id).
//
...
/**
 * Resolve a station label to a validated fat.stations row. Parses the numeric id
 * from the label and confirms the station exists. Returns { id, name } or
 * { id: null, name: null } when unresolved.
 *
 * @param {string|null} label
 * @param {object} client  fat-scoped Supabase client
 * @returns {Promise<{ id: number|null, name: string|null }>}
 */
async function resolveStation(label, client) {
  const parsedId = parseStationInput(label).id
  if (parsedId == null) return { id: null, name: null }
...
export async function mirrorClaimToCanonical({ userId, claimType, date, fields = {}, breakdown = null, routingMeta = null, client = fat }) {
  ...
  try {
    ...
    const rostered = await resolveStation(fields.rosteredStn, client)
    const dest     = await resolveStation(fields.standbyStn, client)
