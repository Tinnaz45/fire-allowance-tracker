...
// ─── Operational-container parent detection ────────────────────────────────────
// AUTHORITATIVE source of truth for "is this row a parent operational record
// whose dollar amounts are duplicated by its generated auto-child entitlements?"
//
// Recall and Retain parents are written with the FULL claim total AND spawn
// auto-children that each carry a split of those same dollars (Callback-Ops,
// Excess Travel, Petty cash meal / Maint stn N/N, Overnight cash, meal). Summing
// the parent and the children double-counts. Such a parent is therefore a pure
// grouping container: its dollars must be ignored (zeroed in groupedView) and the
// children are the truth source. The Standby/M&D parent is NOT a container — it
// is itself a tagged payslip entitlement (calculation_inputs.autoChild =
// 'standby_and_dismi' | 'md_event') and is already written with total_amount = 0.
//
// A row is a container parent iff it carries NO autoChild slug (so it is a parent,
// not a generated entitlement) AND at least one sibling in the same group DOES
// carry an autoChild slug (so generated children exist to hold the dollars). A
// standalone claim (its own group, no auto-children — e.g. a legacy retain/recall
// or a meal claim) is never a container, so its single row keeps its amount.
//
// Used by ClaimsContext.groupedView (to zero the parent's dollar contribution —
// the one place every dollar aggregator reads from) and by the claim-list cards
// (to hide the now-$0 container row). The tax page applies the equivalent
// parent-only rule directly in calcTaxSummary for km/meal counts.

/**
 * @param {object} claim — the candidate row
 * @param {object[]} siblings — all rows sharing the claim's group
 * @returns {boolean}
 */
export function isOperationalContainerRow(claim, siblings) {
