...
// ─── Display label (fire vs non-fire) ────────────────────────────────────────
// The on-screen label is derived from the station RECORD, never the other way
// round. `abbreviation` is the single source of truth for "is this a numbered
// fire station?":
//   - present ("FS45")  → fire station   → "FS45 - Brooklyn"
//   - null / empty      → operational    → "Brooklyn" (name only, no fake FS)
// This is the only function that decides how a station reads to the user, so a
// non-fire location such as Spring Street never shows a fabricated FS number.
//
// IMPORTANT: identity is carried separately (the station id — see stationById).
// Display labels are for humans and audit columns only; nothing parses an id
// back out of them.
export function displayLabelForStation(station) {
  if (!station) return ''
  const name = station.name || ''
  const abbr = station.abbreviation ? String(station.abbreviation).trim() : ''
  if (!abbr) return name
  return name ? `${abbr} - ${name}` : abbr
}

/**
 * Explicit-identity lookup: resolve a station id (number or numeric string)
 * against an already-loaded stations list. This is the canonical replacement
 * for the old "parse an FS number out of the visible label" path — selection,
 * persistence, distance and entitlement code all carry the id and resolve the
 * underlying record through here.
 *
 * Returns the matched record augmented with a derived display `label`, or null
 ...
 * @returns {{ id:number, name:string, abbreviation:string|null, label:string } | null}
 */
export function stationById(id, stations) {
  if (id == null || id === '' || !Array.isArray(stations) || stations.length === 0) return null
  const sid = String(id)
  const s = stations.find((x) => String(x.id) === sid)
  if (!s) return null
  return { ...s, label: displayLabelForStation(s) }
}

/**
 * Build the canonical display label for a station record. Retained for callers
 * that still hold a record (the station picker badge, audit-column writes); it
 * now delegates to displayLabelForStation so fire/non-fire formatting is
 * consistent everywhere and no fabricated "FS{id}" fallback is ever produced.
 */
export function labelForStation(station) {
  return displayLabelForStation(station)
}
