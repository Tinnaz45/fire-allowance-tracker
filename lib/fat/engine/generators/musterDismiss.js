...
//
// ⚠️ ISOLATION — NOT the petty-cash kilometre entitlement.
// The `excess_travel_md` row below is expressed in HOURS (FRV travel-time
// matrix, rostered → M&D station) and exists ONLY for payroll comparison. It is
// deliberately independent of, and must NEVER be conflated with, the M&D
// petty-cash KILOMETRE reimbursement, which is the confirmed rule
//     payable km = max(0, Home→Rostered − Home→M&D)
// computed on the prototype path (components/distance/MdDistanceField.js →
// lib/calculations/engine.js `calcMdPayableKm` → fat.standby.dist_km/travel_amount
// → petty-cash CSV export). Home location is intentionally NOT an input here;
// this generator neither reads nor overrides the petty-cash km. Do not "fix"
// this to use home distances — that would corrupt the payroll-hours comparison.
//
