...
/**
 * Pick the extraction adapter: the REAL provider adapter when a provider key is
 * configured (isOcrConfigured), else the deterministic stub so a keyless DEV box
 * still runs the validated framework. PAYSLIP_OCR_PROVIDER='stub' forces the stub.
...
export function resolveExtractionAdapter() {
  return isOcrConfigured() ? screenshotOcrAdapter : stubScreenshotOcrAdapter
}
