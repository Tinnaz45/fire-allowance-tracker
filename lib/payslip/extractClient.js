...
export async function getOcrAvailability(opts = {}) {
  try {
    // The probe enforces the per-user Payments feature gate server-side, so it
    // requires the access token (like the POST below). Not signed in / no token
    // → fail safe to "unavailable" (extraction hidden).
