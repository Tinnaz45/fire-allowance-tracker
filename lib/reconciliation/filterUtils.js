...
//   claimType        — 'all' | 'recalls' | 'retain' | 'standby' | 'md' | 'spoilt' | 'delayed_meal'
...
export const DEFAULT_FILTERS = {
  ...
  platoon:         'all',    // 'all' | 'A' | 'B' | 'C' | 'D'
  search:          '',       // free-text query (matches type/number/shift/platoon/date/station)
...
/**
 * Returns true if the group matches the shift filter.
 * @param {object} entry — grouped view entry
 * @param {'all'|'Day'|'Night'} shift
 * @returns {boolean}
 */
export function filterGroupByShift(entry, shift) {
  if (shift === 'all') return true
  return resolveGroupShift(entry) === shift
}

/**
 * Returns true if the group matches the platoon filter.
 * @param {object} entry — grouped view entry
 * @param {'all'|'A'|'B'|'C'|'D'} platoon
 * @returns {boolean}
 */
export function filterGroupByPlatoon(entry, platoon) {
  if (platoon === 'all') return true
  return resolveGroupPlatoon(entry) === platoon
}

/**
 * Returns true if the group matches the free-text search query.
 * Matches against claim type label, claim number, shift, platoon, date and
 * station labels of the group and any of its children.
 *
 * @param {object} entry — grouped view entry
 * @param {string} search
 * @returns {boolean}
 */
export function filterGroupBySearch(entry, search) {
  const q = (search || '').trim().toLowerCase()
  if (!q) return true
  return buildGroupSearchHaystack(entry).includes(q)
}
...
// ─── Search Haystack Builders ─────────────────────────────────────────────────
// Build a single lowercase string of everything a user might search a claim by.

function claimSearchTokens(claim) {
  if (!claim) return []
  const tokens = []
  const typeLabel = CLAIM_TYPE_LABELS[claim.claimType] || claim.claimType
  if (typeLabel) tokens.push(typeLabel)
  if (claim.claim_number != null) tokens.push(`#${claim.claim_number}`, String(claim.claim_number))
  const shift = resolveClaimShift(claim)
  if (shift) tokens.push(shift, `${shift} shift`)
  const platoon = resolveClaimPlatoon(claim)
  if (platoon) tokens.push(platoon, `platoon ${platoon}`)
  if (claim.date) tokens.push(formatDateDDMMYY(claim.date), claim.date)
  if (claim.recall_stn_label) tokens.push(claim.recall_stn_label)
  if (claim.rostered_stn_label) tokens.push(claim.rostered_stn_label)
  return tokens
}

export function buildGroupSearchHaystack(entry) {
  const tokens = []
  const group = entry?.group || {}
  const typeLabel = CLAIM_TYPE_LABELS[group.claim_type] || group.claim_type
  if (typeLabel) tokens.push(typeLabel)
  if (group.label) tokens.push(group.label)
  if (group.claim_number != null) tokens.push(`#${group.claim_number}`, String(group.claim_number))
  if (group.incident_number) tokens.push(group.incident_number)
  if (group.incident_date) tokens.push(formatDateDDMMYY(group.incident_date), group.incident_date)
  const shift = resolveGroupShift(entry)
  if (shift) tokens.push(shift, `${shift} shift`)
  const platoon = resolveGroupPlatoon(entry)
  if (platoon) tokens.push(platoon, `platoon ${platoon}`)
  for (const child of entry?.children || []) tokens.push(...claimSearchTokens(child))
  return tokens.join(' ').toLowerCase()
}

export function buildClaimSearchHaystack(claim) {
  return claimSearchTokens(claim).join(' ').toLowerCase()
}
...
/**
 * Returns true if an ungrouped claim matches the shift filter.
 * @param {object} claim
 * @param {'all'|'Day'|'Night'} shift
 * @returns {boolean}
 */
export function filterUngroupedByShift(claim, shift) {
  if (shift === 'all') return true
  return resolveClaimShift(claim) === shift
}

/**
 * Returns true if an ungrouped claim matches the platoon filter.
 * @param {object} claim
 * @param {'all'|'A'|'B'|'C'|'D'} platoon
 * @returns {boolean}
 */
export function filterUngroupedByPlatoon(claim, platoon) {
  if (platoon === 'all') return true
  return resolveClaimPlatoon(claim) === platoon
}

/**
 * Returns true if an ungrouped claim matches the free-text search query.
 * @param {object} claim
 * @param {string} search
 * @returns {boolean}
 */
export function filterUngroupedBySearch(claim, search) {
  const q = (search || '').trim().toLowerCase()
  if (!q) return true
  return buildClaimSearchHaystack(claim).includes(q)
}
...
export function groupPassesFilters(entry, filters) {
  ...
  return (
    ...
    filterGroupByShift(entry, f.shift) &&
    filterGroupByPlatoon(entry, f.platoon) &&
    filterGroupBySearch(entry, f.search) &&
...
export function ungroupedPassesFilters(claim, filters) {
  ...
  return (
    ...
    filterUngroupedByShift(claim, f.shift) &&
    filterUngroupedByPlatoon(claim, f.platoon) &&
    filterUngroupedBySearch(claim, f.search) &&
